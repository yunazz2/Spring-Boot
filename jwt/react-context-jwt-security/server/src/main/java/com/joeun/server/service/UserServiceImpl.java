...
public class UserServiceImpl implements UserService {
    ...
    public void login(Users user, HttpServletRequest requset) throws Exception {
        ...
        // AuthenticationManager(인증 관리자 객체)
        // 아이디, 패스워드 인증 토큰 생성
        UsernamePasswordAuthenticationToken token
            = new UsernamePasswordAuthenticationToken(username, password);

        // 위에 토큰이 만들어져 있어야 그걸 AuthenticationManager로 요청을 보낼 수 있음
        ...
        User authUser = (User) authentication.getPrincipal();   // 이 User는 우리가 만든 User가 아니라 spring security의 User임
